...
```{R YU_CPS comparison}
cps_preprocessed<-fread("/cloud/project/Tutorials/Datasets/cps_df_processed.csv")
...
saveRDS(both, file = "yu_treatment_control.rds")
...
Here we try to see whether there is meaningful heterogeneity using the region_large variable, which is a proxy for the size of a region. It looks like the largest treatment effect occurs in the largest regions (region_large = 4), although the treatment effect does not increase monotonically in region size. Seeing that observations with region_large = 4 have a much larger treatment effect, we run a regression with treatment, a dummy for region_large=4, and the interaction. The interaction term is positive and significant. This indicates that individuals in region_large=4 may benefit the most from YearUp. 

```{r location_effect}
ate<-lm(log_d_earn~treatment, data=subset(both, region_large==1))
summary(ate)
exp(summary(ate)$coefficients[2,1])

ate<-lm(log_d_earn~treatment, data=subset(both, region_large==2))
summary(ate)
exp(summary(ate)$coefficients[2,1])

ate<-lm(log_d_earn~treatment, data=subset(both, region_large==3))
summary(ate)
exp(summary(ate)$coefficients[2,1])

ate<-lm(log_d_earn~treatment, data=subset(both, region_large==4))
summary(ate)
exp(summary(ate)$coefficients[2,1])

both$is_region_large_4 <- both$region_large == 4
ate<-lm(log_d_earn~treatment + is_region_large_4 + is_region_large_4*treatment, data = both)

summary(ate)

paste0("Coefficient on Treatment")
exp(summary(ate)$coefficients[2,1])
paste0("Coefficient on Dummy for region_large=4")
exp(summary(ate)$coefficients[3,1])
paste0("Coefficient on Interaction")
exp(summary(ate)$coefficients[4,1])
```
...
```{r occupation_effect}
cutoff <- median(both$task_abstract)
ate<-lm(log_d_earn~treatment, data=subset(both, task_abstract<cutoff))

summary(ate)
paste0("Treatment effect for low abstract task measure")
exp(summary(ate)$coefficients[2,1])

ate<-lm(log_d_earn~treatment, data=subset(both, task_abstract>=cutoff))

summary(ate)
paste0("Treatment effect for high abstract task measure")
exp(summary(ate)$coefficients[2,1])
...
```{r added interaction effects}
test_1 <-lm(log_d_earn~treatment + task_abstract + sex + occ1digit, data = both)
summary(test_1)

test_2 <-lm(log_d_earn~(treatment + task_abstract + sex + occ1digit)^2, data = both)
summary(test_2)
```
