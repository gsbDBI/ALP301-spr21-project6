...
YU_pre_post <- yu_dfsum %>% 
  group_by(sex) %>%
  summarise(first_year=mean(real_yearly_earnings.1_pre),
            second_year=mean(real_yearly_earnings.2_post))

CPS_pre_post <- cps_dfsum %>%
  group_by(sex) %>%
  summarise(first_year=mean(real_yearly_earnings_1),
            second_year=mean(real_yearly_earnings_2))
